  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (mode === 'register') {
        ...
        if (stripeError) throw new Error(stripeError.message || "Ingresa los datos correctamente.");
        ...
        if (data.requiresAction && data.clientSecret) {
          ...
          transactionId = (result.setupIntent || result.paymentIntent).id;
        }
  ...
  return (
    <>
          ...
          <Box sx={{ position: 'relative', mb: 2 }}>
            ...
            {mode === 'login' && (
              <div style={{ textAlign: 'right', marginTop: '8px', marginBottom: '8px' }}>
                <span onClick={() => { setMode('reset'); setError(''); }} style={{ fontSize: '0.85rem', color: 'var(--pida-accent)', cursor: 'pointer', fontWeight: '500' }}>
          ...
          <div style={{ animation: 'fadeIn 0.3s ease' }}>
            <div style={{ marginBottom: '20px', paddingTop: '10px', borderTop: '1px solid #E2E8F0' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '6px', marginBottom: '15px' }}>
                {['basico', 'avanzado', 'premium'].map((p) => (
                  <button key={p} type="button" onClick={() => { setPlan(p); setDiscountData(null); }} style={{
                    padding: '10px 2px', 
                    borderRadius: '8px', 
                    border: `2px solid ${plan === p ? 'var(--pida-primary)' : '#E2E8F0'}`,
                    background: plan === p ? '#F0F7FF' : 'white', 
                    cursor: 'pointer', 
                    transition: '0.2s', 
                    fontWeight: '600', 
                    color: plan === p ? 'var(--pida-primary)' : '#64748B', 
                    fontSize: '0.8rem', 
                    textAlign: 'center'
                  }}>
                    {p === 'basico' ? 'Básico' : p}
              ...
              <div style={{ display: 'flex', gap: '8px', marginBottom: '15px' }}>
                <button type="button" onClick={() => setInterval('monthly')} style={{ 
                  flex: 1, 
                  padding: '10px', 
                  borderRadius: '8px', 
                  border: `2px solid ${interval === 'monthly' ? 'var(--pida-primary)' : '#E2E8F0'}`, 
                  background: interval === 'monthly' ? '#F0F7FF' : 'white', 
                  cursor: 'pointer', 
                  fontWeight: '600', 
                  color: interval === 'monthly' ? 'var(--pida-primary)' : '#64748B', 
                  fontSize: '0.85rem' 
                }}>
                  Mensual
                </button>
                <button type="button" onClick={() => setInterval('annual')} style={{ 
                  flex: 1, 
                  padding: '10px', 
                  borderRadius: '8px', 
                  border: `2px solid ${interval === 'annual' ? 'var(--pida-primary)' : '#E2E8F0'}`, 
                  background: interval === 'annual' ? '#F0F7FF' : 'white', 
                  cursor: 'pointer', 
                  position: 'relative', 
                  fontWeight: '600', 
                  color: interval === 'annual' ? 'var(--pida-primary)' : '#64748B', 
                  fontSize: '0.85rem' 
                }}>
                  Anual
                  <span style={{ position: 'absolute', top: '-10px', right: '-5px', background: '#10B981', color: 'white', fontSize: '0.55rem', padding: '2px 6px', borderRadius: '8px', fontWeight: '800', border: '1px solid white' }}>
                    AHORRA ~20%
                  </span>
                </button>
              </div>
            ...
            <div className="auth-summary-box" style={{ background: '#F0FDF4', border: '1px solid #BBF7D0', padding: '15px', borderRadius: '10px', marginBottom: '15px' }}>
              <div className="auth-summary-row" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span className="auth-summary-label" style={{ color: '#166534', fontWeight: '600' }}>Total a pagar hoy:</span>
                <span style={{ fontWeight: '800', color: '#166534', fontSize: '1.1rem' }}>$0.00 (Prueba)</span>
              </div>
            </div>
            ...
            <div className="terms-group" style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', marginTop: '15px', marginBottom: '15px' }}>
              ...
              <label style={{ fontSize: '0.8rem', color: '#64748B', lineHeight: '1.4' }}>Acepto los términos de uso.</label>
            </div>
          </div>
        )}

        {error && (
          <div className="status-msg error" style={{ color: '#EF4444', fontSize: '0.85rem', background: '#FEF2F2', padding: '10px', borderRadius: '6px', marginBottom: '15px', border: '1px solid #FECACA' }}>
            {error}
          </div>
        )}
      ...
      <div className="bottom-link" style={{ textAlign: 'center', marginTop: '20px' }}>
        {mode === 'reset' && (
          <span style={{ cursor: 'pointer', color: 'var(--pida-primary)', fontSize: '0.9rem', fontWeight: '500' }} onClick={() => { setMode('login'); setError(''); }}>
            ← Volver al login
          </span>
        )}
        {mode === 'register' && (
...
export default function AuthModal({ isOpen, initialMode = 'login', onClose }) {
  ...
  return (
    <div className="modal-backdrop" onClick={onClose} style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
      <div className="modal-card" onClick={e => e.stopPropagation()} style={{ maxWidth: '450px', width: '90%', padding: '30px', background: 'white', borderRadius: '16px', position: 'relative' }}>
