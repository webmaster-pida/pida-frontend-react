        ...
        {steps.map((s, i) => (
          <React.Fragment key={i}>
            <div style={{ 
              ...
              {s.requirement && <div style={{ fontSize: '0.75rem', color: '#B91C1C', fontWeight: 'bold', marginBottom: '8px' }}>REQ: {s.requirement}</div>}
...
export default function AnalyzerInterface({ user, resetSignal, loadAnaId }) {
  ...
  useEffect(() => { if (isAtBottom) scrollToBottom(); }, [messages, isAnalyzing]);
  useEffect(() => { if (resetSignal > 0) handleClear(); }, [resetSignal]);

  useEffect(() => {
    if (loadAnaId) {
      const loadPastAna = async () => {
        setIsAnalyzing(true); setStatusText('Cargando historial...');
        setErrorModal({ show: false, title: '', message: '' });
        setMessages([]); setCurrentAnaId(null); setIsAtBottom(true);
        try {
          const token = await user.getIdToken();
          const res = await fetch(`${API_ANA}/analysis-history/${loadAnaId}`, { headers: { 'Authorization': `Bearer ${token}` } });
          if (!res.ok) throw new Error("Error del servidor al cargar el historial.");
          const data = await res.json();
          try {
            const parsed = JSON.parse(data.analysis);
            if (Array.isArray(parsed)) setMessages(parsed);
            else setMessages([{ role: 'user', content: data.instructions }, { role: 'model', content: data.analysis }]);
          } catch(e) { setMessages([{ role: 'user', content: data.instructions }, { role: 'model', content: data.analysis }]); }
          setCurrentAnaId(loadAnaId); setFiles([]);
  ...
  const handleFileChange = (e) => {
    if (e.target.files) {
      ...
      selectedFiles.forEach(file => {
        ...
        if (fileSizeMB > 50) {
          setErrorModal({ show: true, title: "Archivo excede el límite máximo", message: `El archivo **${file.name}** pesa ${fileSizeMB.toFixed(2)} MB.\n\nEl límite máximo absoluto permitido en la plataforma es de **50 MB** por documento.` });
        } else validFiles.push(file);
      });
      if (validFiles.length > 0) { setFiles(prev => [...prev, ...validFiles]); setErrorModal({ show: false, title: '', message: '' }); }
    }
  };

  const removeFile = (indexToRemove) => { setFiles(files.filter((_, index) => index !== indexToRemove)); };
  const handleClear = () => { setFiles([]); setInstructions(''); setMessages([]); setCurrentAnaId(null); setErrorModal({ show: false, title: '', message: '' }); setStatusText(''); setIsAtBottom(true); };

  const handleBackendDownload = async (format) => {
    if (messages.length === 0) { alert("Por favor, interactúa en el analizador antes de descargarlo."); return; }
    try {
      const token = await user.getIdToken();
      ...
      const res = await fetch(`${API_ANA}/download-analysis`, { method: 'POST', headers: { 'Authorization': `Bearer ${token}` }, body: formData });
    ...
    } catch (error) { console.error(error); alert("Hubo un problema descargando el archivo."); }
  };

  const handleAnalyze = async (eOrInstruction = null) => {
    if (eOrInstruction && eOrInstruction.preventDefault) eOrInstruction.preventDefault();
    const currentInstruction = typeof eOrInstruction === 'string' ? eOrInstruction : instructions;
    if (files.length === 0 && !currentAnaId) { alert("Por favor, selecciona al menos un documento."); return; }
    setIsAnalyzing(true); setIsAtBottom(true); setTimeout(() => scrollToBottom(), 50);
    setErrorModal({ show: false, title: '', message: '' });
    const newMessages = [...messages, { role: 'user', content: currentInstruction }];
    setMessages(newMessages); setInstructions('');
    try {
      const token = await user.getIdToken();
      let uploadedFilesData = [];
      const smallFiles = []; const largeFiles = [];
      files.forEach(f => {
          const sizeMB = f.size / (1024 * 1024);
          if (sizeMB > 10 && sizeMB <= 50 && f.type === 'application/pdf') largeFiles.push(f);
      ...
      if (smallFiles.length > 0) {
        ...
        const urlRes = await fetch(`${API_ANA}/generate-upload-urls`, { method: 'POST', headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ files: fileMetadata }) });
        if (!urlRes.ok) throw new Error("Error al obtener permisos de almacenamiento.");
        const urlData = await urlRes.json();
        const { urls } = urlData;
        setStatusText('Cargando documentos...');
        const uploadPromises = smallFiles.map((file, i) => fetch(urls[i].upload_url, { method: 'PUT', body: file, headers: { 'Content-Type': file.type || 'application/pdf' } }));
        await Promise.all(uploadPromises);
        urls.forEach(u => uploadedFilesData.push({ gs_uri: u.gs_uri, filename: u.filename, mime_type: u.mime_type }));
      }
      if (largeFiles.length > 0) {
        setStatusText('Optimizando archivos pesados...');
        const optimizePromises = largeFiles.map(async (file) => {
            const formData = new FormData(); formData.append('file', file);
            const optRes = await fetch(`${API_ANA}/compress-and-upload`, { method: 'POST', headers: { 'Authorization': `Bearer ${token}` }, body: formData });
            if (!optRes.ok) throw new Error(`Fallo al optimizar: ${file.name}`);
            return await optRes.json();
        });
        const optimizedResults = await Promise.all(optimizePromises);
        optimizedResults.forEach(res => uploadedFilesData.push({ gs_uri: res.gs_uri, filename: res.filename, mime_type: res.mime_type }));
      }
      setStatusText('Procesando información con IA...');
      const fd = new FormData();
      fd.append('files_data', JSON.stringify(uploadedFilesData)); fd.append('instructions', currentInstruction);
      if (currentAnaId) fd.append('analysis_id', currentAnaId);
      fd.append('history_json', JSON.stringify(newMessages));
      const res = await fetch(`${API_ANA}/analyze`, { method: 'POST', headers: { 'Authorization': `Bearer ${token}` }, body: fd });
      if (!res.ok) throw new Error("Límite de suscripción o error del servidor.");
      const reader = res.body.getReader(); const decoder = new TextDecoder("utf-8");
      let fullText = ""; let streamBuffer = ""; setStatusText('');
      while (true) {
            ...
            try {
              ...
              if (d.text) {
                ...
                setMessages(prev => {
                  ...
                  if (lastMsg && lastMsg.role === 'model') return [...prev.slice(0, -1), { ...lastMsg, content: fullText }];
                  else return [...prev, { role: 'model', content: fullText }];
                });
              }
              if (d.analysis_id) setCurrentAnaId(d.analysis_id);
  ...
  const handleFollowUpClick = (question) => { setInstructions(question); handleAnalyze(question); };
  ...
  const renderAnalysisContent = (text, idx) => {
    ...
    if (parts.length > 1 && !isTyping) {
      ...
      for (const line of lines) {
        ...
        } else if (isLinkOrSource) sources.push(line);
      }
        ...
        <>
          ...
          {questions.length > 0 && (
            <div className="follow-up-section" style={{ marginTop: '25px', paddingTop: '20px', borderTop: '1px solid #E5E7EB' }}>
              <strong style={{ display: 'block', marginBottom: '12px', color: 'var(--pida-primary)', fontSize: '0.95rem', textTransform: 'uppercase' }}>¿Quieres profundizar?</strong>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {questions.map((q, i) => ( <button key={i} className="follow-up-btn" onClick={() => handleFollowUpClick(q)}>{q}</button> ))}
              </div>
            </div>
          )}
          {sources.length > 0 && (
            <div className="markdown-content" style={{ marginTop: '20px', display: 'block', width: '100%', maxWidth: '100%', overflowX: 'hidden', wordBreak: 'break-word' }}>
              <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeRaw]} components={markdownComponents}>{sources.join('\n')}</ReactMarkdown>
            </div>
          )}
        </>
      );
  ...
  return (
    <div className="pida-view" style={{ position: 'relative', maxWidth: '100%', overflowX: 'hidden' }}>
        ...
        <div id="pida-chat-box" style={{ maxWidth: '100%', overflowX: 'hidden' }}> 
            ...
            <div className="pida-bubble pida-message-bubble">
              <div className="pida-welcome-content">
                <div className="pida-welcome-text">
                  ...
                  <p style={{ color: 'var(--text)' }}>Sube tus archivos y escribe una instrucción. PIDA leerá y sistematizará el documento por ti.</p>
                </div>
              </div>
            </div>
          )}
          {messages.map((msg, idx) => (
            <div key={idx} className={`pida-bubble ${msg.role === 'user' ? 'user-message-bubble' : 'pida-message-bubble'}`}
              style={{ maxWidth: '100%', minWidth: 0, display: 'flex', flexDirection: 'column', boxSizing: 'border-box', overflowX: 'hidden', overflowWrap: 'anywhere' }}>
                {msg.role === 'user' 
          ...
          {isAnalyzing && (!messages.length || messages[messages.length - 1].role === 'user') && (
            <div style={{ textAlign: 'center', marginTop: '40px' }}><div className="loader"></div><p style={{ color: 'var(--pida-text-muted)', marginTop: '15px' }}>{statusText || "Procesando..."}</p></div>
          )}
      ...
      {!isAtBottom && messages.length > 0 && (
        <Fab color="primary" size="medium" onClick={(e) => { e.preventDefault(); setIsAtBottom(true); scrollToBottom(); }}
          sx={{ position: 'absolute', bottom: '180px', right: '25px', zIndex: 900, opacity: 0.9, backgroundColor: 'var(--pida-primary)', '&:hover': { backgroundColor: 'var(--pida-accent)', opacity: 1 } }}>
        ...
        {messages.length > 0 && (
          <Box sx={{ display: 'flex', justifyContent: 'flex-end', mb: 1 }}>
            <ButtonGroup size="small" variant="outlined" color="inherit" sx={{ borderColor: '#e2e8f0', bgcolor: 'white' }}>
              <Button sx={{ fontSize: '0.7rem', fontWeight: 600 }} onClick={() => Exporter.downloadTXT(getTimestampedName("Analisis-PIDA"), "Análisis", messages)}>TXT</Button>
              <Button sx={{ fontSize: '0.7rem', fontWeight: 600 }} onClick={() => handleBackendDownload('docx')}>DOCX</Button>
              <Button sx={{ fontSize: '0.7rem', fontWeight: 600 }} onClick={() => handleBackendDownload('pdf')}>PDF</Button>
            </ButtonGroup>
          </Box>
        )}
        <input type="file" multiple accept=".pdf,.doc,.docx" style={{ display: 'none' }} ref={fileInputRef} onChange={handleFileChange} />
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
          <button type="button" className="pida-header-btn primary" style={{ width: 'auto' }} onClick={() => fileInputRef.current.click()} disabled={isAnalyzing}>+ Seleccionar Archivos</button>
        </div>
        {files.length > 0 && (
          <div id="active-files-area" style={{ display: 'flex', gap: '5px', flexWrap: 'wrap', marginBottom: '10px' }}>
            {files.map((file, idx) => ( <div key={idx} className="active-file-chip"><span>{file.name}</span><button type="button" onClick={() => removeFile(idx)} disabled={isAnalyzing}>×</button></div> ))}
          </div>
        )}
        <TextField id="user-instructions" multiline minRows={2} maxRows={5} fullWidth placeholder="Instrucciones para el análisis..." value={instructions} onChange={(e) => setInstructions(e.target.value)}
          onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) handleAnalyze(e); }} disabled={isAnalyzing} sx={{ mb: 2, '& .MuiOutlinedInput-root': { backgroundColor: '#FAFAFA', borderRadius: 2 } }} />
        <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
          <Button variant="text" onClick={handleClear} disabled={isAnalyzing} sx={{ color: 'text.secondary', fontWeight: 500 }}>Limpiar</Button>
          <Button type="submit" variant="contained" color="primary" disabled={isAnalyzing} sx={{ width: 220, py: 1.2, borderRadius: 2, fontWeight: 600, bgcolor: 'var(--pida-primary)' }}>Analizar</Button>
        </Box>
      </form>
      ...
      {showMissingFileModal && (
        <div className="modal-backdrop" style={{ zIndex: 999999 }} onClick={() => setShowMissingFileModal(false)}>
          <div className="modal-card" onClick={e => e.stopPropagation()} style={{ padding: '40px 30px', maxWidth: '420px', borderRadius: '16px' }}>
            ...
            <p className="modal-subtitle" style={{ fontSize: '0.95rem', lineHeight: '1.6', marginBottom: '30px', color: '#4B5563' }}>Para continuar, por favor vuelve a adjuntar el documento original.</p>
        ...
        <div className="modal-backdrop" style={{ zIndex: 999999 }} onClick={() => setErrorModal({ show: false, title: '', message: '' })}>
          <div className="modal-card" onClick={e => e.stopPropagation()} style={{ padding: '40px 30px', maxWidth: '440px', borderRadius: '16px', border: '1px solid #FECACA' }}>
            ...
            <div className="modal-subtitle markdown-content" style={{ fontSize: '0.95rem', lineHeight: '1.6', marginBottom: '30px', color: '#4B5563', textAlign: 'left' }}>
              <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeRaw]}>{errorModal.message}</ReactMarkdown>
            </div>
